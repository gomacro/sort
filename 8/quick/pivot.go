...
func medianOfThree(ts0 *[1]uintptr, data []uint8, a, b, c int, compar func(*uint8, *uint8) int) {
	incr := int((*ts0)[0])
...
func swapRange(ts0 *[1]uintptr, data []uint8, a, b, n int) {
	incr := int((*ts0)[0])
...
func doPivot(ts0 *[1]uintptr, compar func(*uint8, *uint8) int, data []uint8) (midlo, midhi int) {
	incr := int((*ts0)[0])
